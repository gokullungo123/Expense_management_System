...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
        ...
        Integer findTotalNoOfTransactionsByUser(@Param("userId") long userId, @Param("month") int month,
                        @Param("year") int year);
        ...
        @Query(value = "SELECT " +
                        ...
                        "JOIN category c on t.category_id = c.category_id " +
                        "JOIN transaction_type tt on c.transaction_type_id = tt.transaction_type_id " +
                        "WHERE u.email = :email and t.date >= DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH) " +
                        "GROUP BY YEAR(t.date), MONTH(t.date)", nativeQuery = true)
        List<Object[]> findMonthlySummaryByUser(@Param("email") String email);

        @Query(value = "SELECT " +
                        "date,description, category_name, amount " +
                        "FROM transaction t " +
                        "JOIN users u ON t.user_id = u.id " +
                        "JOIN category c ON t.category_id = c.category_id " +
                        "JOIN transaction_type tt ON c.transaction_type_id = tt.transaction_type_id " +
                        "WHERE u.email = :email AND MONTH(t.date) = :month " + 
                        "ORDER BY date DESC", nativeQuery = true)
        List<Object[]> findMonthlyTransactionsByUser(@Param("email") String email, @Param("month") int month);
}